...
**Pre-validated during 1b** (smoke-tested in a throwaway git fixture; the fixtures formalize and extend these): the create → dispatch → land → accept lifecycle on a non-`main` source-of-truth branch; `frozen_hash`/`manifest_hash`/`manifest_sha`/`instructions_hash` stamping + freeze re-check; §4 review linkage (accept blocked until an independent review lands; review handoffs exempt from needing their own review); §8 approval gate (dispatch blocked until an `APPROVALS_LOG.md` record exists); state/index generation; data-driven manifest resolution (actors/roles/branch/layout/protection/limits).
